...
# ── KPI 목표 매출 로드 ──
_kpi_targets = load_kpi_target()
_current_years = sorted(set(
    int(m.split("-")[0]) for m in selected_months if "-" in m
), reverse=True)
_target_year = _current_years[0] if _current_years else datetime.now().year
_target_sales = _kpi_targets.get(_target_year, None)
_achievement_rate = (total_sales / _target_sales * 100) if _target_sales else None
...
c1, c2, c3, c4, c5, c6, c7 = st.columns([1.4, 1, 1.4, 1, 1.2, 1.4, 1])
...
if _target_sales is not None:
    c6.metric(f"목표 매출 ({_target_year})", _fmt_money(_target_sales))
    c7.metric("달성률", f"{_achievement_rate:.1f}%")
else:
    c6.metric(f"목표 매출 ({_target_year})", "목표 미설정")
    c7.metric("달성률", "-")
...
if st.session_state["role"] == "admin":
    with admin_tab:
        ...
        admin_sub = st.tabs(["👥 계정 권한 관리", "➕ 사용자 역할 변경", "🗑️ 계정 관리", "🎯 KPI 목표 설정"])
        ...
        # ── KPI 목표 설정 탭 ──
        with admin_sub[3]:
            st.markdown("### 🎯 연간 목표 매출 설정")
            st.caption("설정한 목표 매출액은 상단 KPI 영역에 달성률과 함께 표시됩니다.")

            _kpi_existing = load_kpi_target()

            _kpi_year = st.selectbox(
                "연도 선택",
                options=[2024, 2025, 2026, 2027],
                index=2,
                key="kpi_target_year"
            )
            _kpi_default = int(_kpi_existing.get(_kpi_year, 0))
            _kpi_input = st.number_input(
                f"{_kpi_year}년 목표 매출액 (원)",
                min_value=0,
                value=_kpi_default,
                step=100000000,
                format="%d",
                key="kpi_target_input"
            )
            st.caption(f"입력값: **{_kpi_input:,.0f} 원**")

            if st.button("💾 목표 매출 저장", use_container_width=True, type="primary", key="kpi_save_btn"):
                try:
                    save_kpi_target(_kpi_year, float(_kpi_input))
                    st.success(f"✅ {_kpi_year}년 목표 매출액 {_kpi_input:,.0f}원이 저장되었습니다.")
                    st.rerun()
                except Exception as _e:
                    st.error(f"❌ 저장 실패: {_e}")
            ...
            # 현재 저장된 목표값 표시
            if _kpi_existing:
                st.markdown("#### 현재 저장된 목표값")
                _kpi_disp = pd.DataFrame([
                    {"연도": y, "목표매출액": f"{v:,.0f} 원"}
                    for y, v in sorted(_kpi_existing.items())
                ])
                st.dataframe(_kpi_disp, use_container_width=True, hide_index=True)
            else:
                st.info("저장된 목표값이 없습니다. KPI_TARGET 시트가 자동 생성됩니다.")
...
st.success("🚀 Lingtea Dashboard v9.0 Ready")
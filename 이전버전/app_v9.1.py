...
_selected_year = st.sidebar.selectbox("📅 분석 연도", options=_available_years, index=0, key="selected_year")
...
st.sidebar.markdown("**📅 세부 기간 설정**")

# 날짜 입력 (해당 연도 범위 내로 제한)
_date_start = st.sidebar.date_input(
    "시작일", 
...
_date_end = st.sidebar.date_input(
    "종료일", 
...
if "경영진요약" in tab_map:
    with tab_map["경영진요약"]:
        ...
        with st.expander("📈 월별 매출 및 공헌이익 추이", expanded=True):
            ...
            _exec_fig = go.Figure()
            # 월별추이 탭과 동일한 색상 (#1f77b4 파란 바, #EF4444 빨간 라인)
            _exec_fig.add_trace(go.Bar(
                x=_exec_monthly["출고년월"], y=_exec_monthly["품목별매출(VAT제외)"],
                name="매출액", marker_color="#1f77b4",
                text=_exec_monthly["품목별매출(VAT제외)"].apply(lambda v: f"{v/1e8:.1f}억"),
                textposition="outside", cliponaxis=False,
                textfont=dict(size=12, color="black"),
            ))
            _exec_fig.add_trace(go.Scatter(
                x=_exec_monthly["출고년월"], y=_exec_monthly["공헌이익"],
                name="공헌이익", mode="lines+markers+text",
                line=dict(width=4, color="#EF4444"),
                text=_exec_monthly["공헌이익"].apply(lambda v: f"{v/1e8:.1f}억"),
                textposition="top center",
                textfont=dict(size=12, color="black"),
            ))
            _exec_monthly_ymax = max(
                _exec_monthly["품목별매출(VAT제외)"].max() if not _exec_monthly.empty else 1,
                _exec_monthly["공헌이익"].max() if not _exec_monthly.empty else 1,
            ) * 1.25
            _exec_fig.update_layout(
                height=380, barmode="group",
                yaxis=dict(range=[0, _exec_monthly_ymax]),
        ...
        # 섹션 7: AI 경영 종합 분석
        # ══════════════════════════════════════
        with st.expander("🤖 AI 경영 종합 분석", expanded=True):
            st.caption("현재 필터 기준 데이터를 AI가 분석하여 C레벨 보고용 인사이트를 제공합니다.")

            def _exec_build_ai_context(fdf):
                lines = []
                ...
                _ech = fdf.groupby("거래처분류")[["품목별매출(VAT제외)", "공헌이익"]].sum().sort_values("품목별매출(VAT제외)", ascending=False).head(10)
                lines.append("\n## 채널별 매출·공헌이익 (상위 10개)")
                ...
                for _in2, _ir in _eig.iterrows():
                    ...
                    lines.append(f"- {_in2}: 매출 {_is:,.0f}원 / 공헌이익 {_ici:,.0f}원 ({_ici/_is*100:.1f}%) / 출고 {_ir['총내품출고수량']:,.0f}" if _is else f"- {_in2}: 매출 0")
                _emkt = fdf.groupby("국내여부")[["품목별매출(VAT제외)", "공헌이익"]].sum()
                lines.append("\n## 국내/해외 구분")
                for _mk, _mr in _emkt.iterrows():
                    _ms3 = _mr["품목별매출(VAT제외)"]; _mci3 = _mr["공헌이익"]
                    lines.append(f"- {_mk}: 매출 {_ms3:,.0f}원 / 공헌이익 {_mci3:,.0f}원 ({_mci3/_ms3*100:.1f}%)" if _ms3 else f"- {_mk}: 매출 0")
                return "\n".join(lines)

            def _exec_call_claude(system_p, messages, max_tokens=2048):
            ...
            _exec_ctx_key = f"exec_ai_ctx_{hash(str(sorted(selected_months)))}_{hash(str(sorted(selected_channel_groups)))}"
            if st.session_state.get("exec_ai_context_key") != _exec_ctx_key:
                st.session_state["exec_ai_context_key"] = _exec_ctx_key
                st.session_state["exec_ai_context"]     = _exec_build_ai_context(filtered_df)
...
- 마크다운 형식으로 구조화하세요."""
            ...
            if _exec_run or st.session_state.get("exec_ai_report") == "__running__":
                ...
                with st.spinner("AI가 경영진 보고용 분석을 생성 중입니다... (15~30초 소요)"):
                    try:
                        _exec_report = _exec_call_claude(
                            _exec_system,
                            [{"role": "user", "content": (

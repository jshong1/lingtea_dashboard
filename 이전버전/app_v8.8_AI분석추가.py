...
if "AI분석" in tab_map:
    with tab_map["AI분석"]:
        st.subheader("🤖 AI 분석")
        st.caption("현재 필터 기준 데이터를 종합 분석합니다. 자동 리포트 생성 후 추가 질문도 가능합니다.")

        # ── 컨텍스트 데이터 준비 (집계 요약본만 전달) ──
        def build_ai_context(fdf):
            ...
            # 월별 추이
            monthly = fdf.groupby("출고년월")[["품목별매출(VAT제외)", "공헌이익"]].sum().reset_index()
            monthly = monthly.sort_values("출고년월")
            lines.append(f"\n## 월별 매출 및 공헌이익 추이")
            for _, row in monthly.iterrows():
                s = row["품목별매출(VAT제외)"]
                ci = row["공헌이익"]
                ci_rate = ci/s*100 if s else 0
                lines.append(f"- {row['출고년월']}: 매출 {s:,.0f}원 / 공헌이익 {ci:,.0f}원 ({ci_rate:.1f}%)")

            # 채널별 요약
            ch = fdf.groupby("거래처분류")[["품목별매출(VAT제외)", "공헌이익"]].sum()
            ch = ch.sort_values("품목별매출(VAT제외)", ascending=False).head(10)
            lines.append(f"\n## 채널별 매출·공헌이익 (상위 10개)")
            for ch_name, row in ch.iterrows():
                s = row["품목별매출(VAT제외)"]
                ci = row["공헌이익"]
                ci_rate = ci/s*100 if s else 0
                lines.append(f"- {ch_name}: 매출 {s:,.0f}원 / 공헌이익 {ci:,.0f}원 ({ci_rate:.1f}%)")

            # 품목군별 요약
            ig = fdf.groupby("품목군")[["품목별매출(VAT제외)", "공헌이익", "총내품출고수량"]].sum()
            ig = ig[~ig.index.isin(["__매출조정__", "__미분류__"])]
            ig = ig.sort_values("품목별매출(VAT제외)", ascending=False)
            lines.append(f"\n## 품목군별 매출·공헌이익")
            for ig_name, row in ig.iterrows():
                s = row["품목별매출(VAT제외)"]
                ci = row["공헌이익"]
                ci_rate = ci/s*100 if s else 0
                lines.append(f"- {ig_name}: 매출 {s:,.0f}원 / 공헌이익 {ci:,.0f}원 ({ci_rate:.1f}%) / 출고 {row['총내품출고수량']:,.0f}")
            ...
            for mkt_name, row in mkt.iterrows():
                ...
                ci = row["공헌이익"]
                ci_rate = ci/s*100 if s else 0
                lines.append(f"- {mkt_name}: 매출 {s:,.0f}원 / 공헌이익 {ci:,.0f}원 ({ci_rate:.1f}%)")
        ...
        # ── 컨텍스트 캐시 (필터 바뀔 때마다 재생성) ──
        _ctx_key = f"ai_context_{hash(str(sorted(selected_months)))}_{hash(str(sorted(selected_channel_groups)))}"
        if "ai_context_key" not in st.session_state or st.session_state["ai_context_key"] != _ctx_key:
        ...
        _system_prompt = f"""당신은 링티(Lingtea) 비즈니스 데이터 분석 전문가입니다.
...
- 마크다운 형식으로 구조화해서 작성하세요."""
        ...
        # ── 1. 자동 분석 리포트 ──
        with st.expander("📋 AI 자동 분석 리포트", expanded=True):
            if st.session_state["ai_auto_report"] is None:
                    ...
                    try:
                        report = call_claude_api(
                            ...
                            [{"role": "user", "content": (
                                ...
                                "1. 📊 핵심 요약 (전체 성과 한 줄 평가)\n"
                                "2. 📈 주목할 매출 트렌드 (월별 변화, 이상치)\n"
                                "3. 🏪 채널 분석 (상위/하위 채널, 공헌이익 관점)\n"
                                "4. 📦 품목군 분석 (성과 좋은/나쁜 품목군)\n"
                                "5. ⚠️ 리스크 & 기회 요인\n"
                                "6. 💡 추천 액션 (구체적으로 2~3가지)"
                            )}]
                        )
                        ...
                        st.session_state["ai_auto_report"] = ""

            if st.session_state["ai_auto_report"]:
                st.markdown(st.session_state["ai_auto_report"])

            col_refresh, _ = st.columns([1, 4])
            with col_refresh:
                if st.button("🔄 리포트 재생성", use_container_width=True):
                    st.session_state["ai_auto_report"] = None
                    st.rerun()

        # ── 2. 추가 질문 채팅 ──
        st.markdown("### 💬 추가 질문")
        st.caption("데이터에 대해 궁금한 점을 자유롭게 질문하세요.")

        # 예시 질문 버튼
        example_questions = [
            "공헌이익률이 가장 낮은 채널은 어디고 원인이 뭐야?",
            "전월 대비 매출 증감이 가장 큰 품목군은?",
            "해외 채널 성과를 국내와 비교해줘",
            "어떤 채널에 집중하면 공헌이익이 가장 많이 늘어날까?",
        ]
        st.markdown("**빠른 질문:**")
        ...
        if user_input:
            ...
            with st.chat_message("assistant"):
                with st.spinner("분석 중..."):
                    try:
                        # 히스토리 최대 10턴 유지 (토큰 절약)
                        recent_history = st.session_state["ai_chat_history"][-10:]
                        answer = call_claude_api(_system_prompt, recent_history)
                        st.markdown(answer)
...
st.success("🚀 Lingtea Dashboard v8.8 Ready")